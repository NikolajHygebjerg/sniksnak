...
export async function POST(request: NextRequest) {
  ...
  if (!token || !supabaseUrl || !serviceRoleKey) {
    return NextResponse.json({ error: "Uautoriseret eller server ikke konfigureret" }, { status: 401 });
  }
  ...
  if (!child_id || !contact_user_id || contact_user_id !== caller.id) {
    return NextResponse.json({ error: "Ugyldig eller uautoriseret anmodning" }, { status: 400 });
  }
